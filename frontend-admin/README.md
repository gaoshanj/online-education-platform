...
## 1. 项目简介  
本项目为《基于SpringBoot + Vue3的在线教育平台》的**管理员端**（后台管理系统）前端代码。系统采用 Vue 3 + Vite + Element Plus 技术栈构建，为平台管理员提供课程管理、用户管理、讲师审核、数据统计等核心后台管控功能。本前端项目与独立开发的服务端 API 进行数据交互，实现了完全的前后端分离。
...
1. **下载源码**
   你可以通过 Git 克隆或直接下载 ZIP 压缩包的方式获取本项目的源码。
   Gitee 仓库地址：[https://gitee.com/feng_s/online-education-admin](https://gitee.com/feng_s/online-education-admin)
   
   使用 Git 命令行克隆：
   ```bash
   git clone https://gitee.com/feng_s/online-education-admin.git
   ```
...
**验证方法**：  
若页面成功渲染出后台系统的全局登录界面，输入下方的默认测试管理员账号进行登录测试。登录成功后能够跳转至后台首页数据统计面板并正常拉取数据，即代表前端已成功启动并完美对接后端服务与数据库系统。

## 7. 默认测试账号

系统在数据库初始化时，提供了一个具有最高权限的超级管理员账号供测试使用：
...
**3. 端口被占用**
- **现象特征**：控制台打印红字提示 `Error: listen EADDRINUSE: address already in use :::5175`。
- **排查解决**：意味着你电脑的 5175 端口正在被别的程序占用使用。可通过在 `vite.config.js` 中的 `server: { port: 5176 }` 修改为一个未被占用的新端口号即可重新启动。
