...
const loadCurrentUserInfo = async () => {
  try {
    let resData = authStore.userInfo
    if (!resData) {
      resData = await authStore.fetchUserInfo()
    }
    if (resData) {
      infoForm.avatar = resData.avatar || ''
      infoForm.nickname = resData.nickname || ''
      infoForm.gender = resData.gender !== undefined ? resData.gender : 0
      
      originalUsername.value = resData.username || ''
      originalEmail.value = resData.email || ''
    }
  } catch (err) {
    console.error(err)
  }
