...
## 1. 项目简介  
本项目为《基于SpringBoot + Vue3的在线教育平台》的**讲师端**前端代码。系统采用 Vue 3 + Vite + Element Plus 技术栈构建，为平台入驻讲师提供课程发布、视频上传、学员管理、问答互动与数据统计等教学管理功能。本前端项目与独立开发的服务端 API 进行数据交互，实现了完全的前后端分离。
...
**验证方法**：  
若页面成功渲染出讲师端的登录界面，输入下方的默认测试账号或已审核通过的讲师账号进行登录测试。登录成功后能够跳转至讲师工作台面板并正常拉取数据，即代表前端已成功启动并完美对接后端服务与数据库系统。

## 7. 默认测试账号

系统在数据库初始化时，通常会提供预设的讲师账号供测试使用（具体以实际数据为准）：
...
**3. 端口被占用**
- **现象特征**：控制台打印红字提示 `Error: listen EADDRINUSE: address already in use :::5174`。
- **排查解决**：意味着你电脑的 5174 端口正在被别的程序占用使用。可通过在 `vite.config.js` 中的 `server: { port: 5176 }` 修改为一个未被占用的新端口号即可重新启动。
